...
class CreateFinalKernel:
        ...
        if len(self.edges) > 0:
            ...
            for edge in self.edges:
                ...
                for p in prototypical_prepositions:
                    ...
                    if (
                            (not is_label_verb(first_word) and re.search(r"\b"+p+r"\b", edge.edgeLabel.named_entity) and p != edge.edgeLabel.named_entity and not case_in_props(dict(edge.target.properties)))
                            or
                            (edge.edgeLabel.named_entity.endswith('ing') and not self.node_functions.check_node_coordinations_for_auxiliary(edge, self.edges) and edge.source.type != "existential")
                    ):
                ...
                if not found_prototypical_prepositions and is_label_verb(edge.edgeLabel.named_entity) and is_kernel_in_props(edge.source) and edge.target.type != 'existential':
        ...
        for node_id in filtered_top_node_ids:
            ...
            while edge_to_loop[0]:
                ...
                filtered_edges = [
                    ...
                    ((x.source.id in descendant_node_ids and x.target.id in descendant_node_ids) or (x.target.id in found_proposition_labels))
                    and
                    ((x.source.id, x.target.id) not in used_edges or ((x.source.id, x.target.id) in used_edges and len(found_proposition_labels) > 0))
                ]
                ...
                kernel, edge_to_loop = create_sentence_obj(filtered_edges, descendant_nodes, self.negations, node_id, found_proposition_labels, self.node_functions, edge_to_loop)
                ...
                self.nodes = {k: Singleton.strip_root_properties(v) if v.id in descendant_node_ids else v for k, v in self.nodes.items()}
    ...
    def kernel_post_processing(self, kernel, position_pairs):
        ...
        if kernel.id in position_pairs and kernel.kernel.edgeLabel is not None and check_semi_modal(kernel.kernel.edgeLabel.named_entity):
            ...
            kernel, edge_to_loop = create_sentence_obj(new_edges, self.nodes, self.negations, root_id, {}, self.node_functions, [False, None])
            kernel = self.kernel_post_processing(kernel, position_pairs)
        return kernel
    ...
    def remove_duplicate_properties(self, kernel, kernel_nodes=None):
        ...
        if 'SENTENCE' in properties_to_keep:
            ...
            if len(sentence_properties) > 1: # Check for more than one SENTENCE in props
                equal_kernels = [sentence for sentence in sentence_properties if sentence.kernel == sentence_properties[0].kernel]
