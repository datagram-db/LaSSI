...
class NodeFunctions:
    ...
    def create_existential_node(self, G, node_id=-1):
        if node_id == -1:
            node_id = self.fresh_id()
        G.add_node(node_id, data=Singleton(
            id=node_id,
    ...
    def check_node_coordinations_for_auxiliary(self, current_edge, G):
        if 'cc' in dict(G.nodes[current_edge[0]]['data'].properties):
            return any(map(has_auxiliary, [edge[2]['label'].named_entity for edge in G.edges(data=True) if edge[0] == current_edge[0]]))
        else:
            return has_auxiliary(current_edge[2]['label'].named_entity)
