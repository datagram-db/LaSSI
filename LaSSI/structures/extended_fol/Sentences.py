...
class FVariable(Formula):
    ...
    def __str__(self):
        ...
        if self.specification is not None:
            negation = "~ " if self.spec_negation else ""
            name += (" [\\textup{of}] " + negation + str(self.specification))
    ...
    def removePropertiesFrom(self, coll, onMatch=False):
        ...
        return FVariable(self.name, self.type, self.specification,
                         self.cop.removePropertiesFrom(coll, onMatch) if self.cop is not None else None,
                         matched=self.matched, id=self.id, properties=prop)
    ...
    def matchWith(self, f, d, ancestor, fugitive):
        ...
        if self.meta == f.meta:
            ...
            if name_test and type_test and specification:
                ...
                var = FVariable(name_value, type_value, spec_value, cop=cop, matched=True, id=self.id,
    ...
    def replaceWith(self, map, onObjectId=False, d=None, fugitive=None, forAllProperties=None):
        ...
        var = FVariable(name=name, type=type, specification=specification, cop=cop, matched=self.matched, id=self.id,
    ...
    def strippedByType(self):
        return FVariable(name=self.name, type="TODO", specification=self.specification, cop=stripArg(self.cop),
                         matched=self.matched, id=self.id, properties=self.properties)
...
class FUnaryPredicate(Formula):
    ...
    def removePropertiesFrom(self, coll, onMatch=False):
        if (not onMatch) or (self.matched):
            return FUnaryPredicate(self.rel,
                                   self.arg.removePropertiesFrom(coll, onMatch) if self.arg is not None else None,
        ...
        if self.meta == f.meta:
            ...
            if rel_test:
                ...
                if arg is None or arg.matched:
                    var = FUnaryPredicate(rel_value, arg, score,
                                          match_frozen_set(self.properties, f.properties, d, ancestor, fugitive),
                                          matched=True)
                    if isAncestorNone:
                        retaliate_dd(d, self, var, fugitive)
                    return var
    ...
    def strippedByType(self):
        return FUnaryPredicate(rel=self.rel, arg=stripArg(self.arg), score=self.score,
                               properties=stripFrozenProperties(self.properties))
...
class FBinaryPredicate(Formula):
    ...
    def matchWith(self, f, d, ancestor, fugitive):
        ...
        if self.meta == f.meta:
            ...
            if rel_test:
                ...
                if (src is None or src.matched) and (dst is None or dst.matched):
                    var = FBinaryPredicate(rel_value, src, dst, score,
                                           match_frozen_set(self.properties, f.properties, d, ancestor, fugitive),
                                           matched=True)
                    if isAncestorNone:
                        retaliate_dd(d, self, var, fugitive)
        ...
        return FBinaryPredicate(self.rel, src, dst, self.score,
                                match_frozen_set(self.properties, f.properties, d, ancestor, fugitive))

    def updateWithProperties(self, toFrozenSet):
        r = {k: tuple(v) for k, v in toFrozenSet.items()}
        return FBinaryPredicate(rel=self.rel, src=self.src, dst=self.dst, score=self.score,
                                properties=frozenset(r.items()), matched=self.matched)
    ...
    def strippedByType(self):
        return FBinaryPredicate(rel=self.rel, src=stripArg(self.src), dst=stripArg(self.dst), score=self.score,
                                properties=stripFrozenProperties(self.properties))
...
class FAnd(Formula):
    ...
    def removePropertiesFrom(self, coll, onMatch=False):
        return FAnd(args=tuple([x.removePropertiesFrom(coll, onMatch) if x is not None else None for x in self.args]),
                    matched=self.matched)
    ...
    def replaceWith(self, m, onObjectId=False, d=None, fugitive=None, forAllProperties=None):
        ...
        if onObjectId and id(self) == m["obj"]:
            onObjectId = False
        var = FAnd(
            args=tuple(map(lambda x: replace_formula(x, m, onObjectId, d, fugitive, forAllProperties), self.args)),
            matched=self.matched)
        retaliate_dd(d, self, var, fugitive)
        return var
...
class FOr(Formula):
    ...
    def removePropertiesFrom(self, coll, onMatch=False):
        return FOr(args=tuple([x.removePropertiesFrom(coll, onMatch) if x is not None else None for x in self.args]),
                   matched=self.matched)
...
class FNot(Formula):
    ...
    def removePropertiesFrom(self, coll, onMatch=False):
        return FNot(arg=self.arg.removePropertiesFrom(coll, onMatch) if self.arg is not None else None,
                    matched=self.matched)
